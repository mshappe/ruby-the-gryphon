...
namespace :roles do
  ...
  task :add, %i[email role] => [:environment] do |_t, args|
    raise ArgumentError, 'No email specified' if args[:email].blank?
    raise ArgumentError, 'No role specified' if args[:role].blank?
